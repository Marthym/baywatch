...
class ScrapingErrorPersistenceAdapterTest {
    ...
    void should_persist_feed_scraping_errors(DSLContext dsl) {
        ...
        List<Entity<ScrapingError>> toPersist = List.of(
                Entity.identify(new ScrapingError(ScrapingExceptionCode.NEED_ACCOUNT, now, now)).withId(FeedSamples.JEDI.id()),
                Entity.identify(new ScrapingError(ScrapingExceptionCode.NEED_ACCOUNT, now, now)).withId(FeedSamples.UNSECURE_PROTOCOL.id())
        );
        ...
        StepVerifier.create(tested.persist(toPersist))
                .assertNext(jedi -> SoftAssertions.assertSoftly(softly -> {
                    ...
                    softly.assertThat(jedi.self().code()).isEqualTo(ScrapingExceptionCode.NEED_ACCOUNT);
                ...
                .assertNext(second -> SoftAssertions.assertSoftly(softly -> {
                    ...
                    softly.assertThat(second.self().code()).isEqualTo(ScrapingExceptionCode.NEED_ACCOUNT);
    ...
    void should_list_scraping_errors(WithSampleDataLoaded.Tracker tracker) {
        ...
        StepVerifier.create(tested.list(QueryContext.all(Criteria.none())))
                .assertNext(jedi -> SoftAssertions.assertSoftly(softly -> {
                    softly.assertThat(jedi.id()).isEqualTo(FeedSamples.JEDI.id());
                    softly.assertThat(jedi.self().code()).isEqualTo(ScrapingExceptionCode.NOT_FOUND);
                ...
                .assertNext(second -> SoftAssertions.assertSoftly(softly -> {
                    ...
                    softly.assertThat(second.self().code()).isEqualTo(ScrapingExceptionCode.NOT_FOUND);
