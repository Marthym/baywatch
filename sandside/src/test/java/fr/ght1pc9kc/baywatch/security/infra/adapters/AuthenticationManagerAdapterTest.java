...
class AuthenticationManagerAdapterTest {
    ...
    private final UserServiceAdapter mockUserService = mock(UserServiceAdapter.class);
    ...
    @Test
    void should_authenticate_unvalidated_user() {
        doReturn(Mono.just(new BaywatchUserDetails(OBIWAN.convert(User::withRoles)))).when(mockUserService).findByUsername(anyString());
        StepVerifier.create(tested.authenticate(new AuthenticationRequest(OBIWAN.self().login(), "nawibo", false)))
